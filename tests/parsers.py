...
class TestMirkotikParserWifiwave2(BaseTests.Parser):
    @classmethod
    def setUpClass(cls):
        cls.connected = MikrotikParser().parse(
            "wireless,info 84:6C:6A:F9:D0:0F@wifi-network connected, signal strength -41"
        )
        cls.disconnected = MikrotikParser().parse(
            "wireless,info 84:6C:6A:F9:D0:0F@wifi-network disconnected, connection lost, signal strength -42"
        )
